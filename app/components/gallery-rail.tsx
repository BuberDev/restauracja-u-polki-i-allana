    ...
    <>
      <div className="mt-10 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <button
          ...
          className="group inline-flex min-h-11 w-fit items-center gap-3 rounded-[8px] border border-white/14 bg-white/8 px-4 py-2 text-sm font-semibold text-white/78 backdrop-blur-md transition hover:border-[#d8f2c2]/45 hover:bg-white/12 hover:text-white focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#d8f2c2]"
        >
          <span className="gallery-cue-icon flex h-7 w-7 items-center justify-center rounded-[8px] bg-[#d8f2c2]/12 text-[#d8f2c2]">
        ...
        <div className="h-1.5 w-full overflow-hidden rounded-full bg-white/10 sm:max-w-64" aria-hidden="true">
          <div
            className="gallery-progress h-full rounded-full bg-[#d8f2c2]"
        ...
        <div className="flex w-max gap-4">
          {items.map((item, index) => (
            <button
              key={item.title}
              type="button"
              className="group block w-[82vw] max-w-[520px] shrink-0 snap-start text-left focus-visible:outline focus-visible:outline-2 focus-visible:outline-offset-4 focus-visible:outline-[#d8f2c2] sm:w-[52vw] lg:w-[420px] xl:w-[470px]"
              onClick={() => {
                setSelectedIndex(index);
                setOpen(true);
              }}
            >
              <Card className="overflow-hidden rounded-[8px] border-white/10 bg-white/6 p-0 text-white shadow-[0_24px_80px_rgba(0,0,0,0.30)] backdrop-blur-sm transition duration-500 group-hover:-translate-y-1 group-hover:border-[#d8f2c2]/35 group-hover:bg-white/10">
                <CardContent className="p-0">
                  <AspectRatio ratio={4 / 3} className="relative aspect-[4/3] overflow-hidden bg-black">
                    <Image
                      ...
                      priority={index === 0}
                    />
                    <div className="absolute inset-0 bg-gradient-to-t from-[#071007]/88 via-[#071007]/18 to-transparent" />
      ...
      {selected ? (
        <Dialog open={open} onOpenChange={setOpen}>
          <DialogContent className="max-w-6xl border-white/10 bg-[#071007] p-2 text-white sm:max-w-6xl">
